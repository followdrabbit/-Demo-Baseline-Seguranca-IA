...
vendor = st.selectbox("Selecione o Vendor", ["Selecione uma opção"] + list(config.keys()))
if vendor != "Selecione uma opção":
    tecnologias_disponiveis = list(config[vendor].keys())
    ...
    if tecnologia != "Selecione uma opção":
        tecnologia_info = config[vendor].get(tecnologia)
